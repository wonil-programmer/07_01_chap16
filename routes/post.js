          ...
          mongodb
            ...
            .catch((err) => {
              res.render("index.ejs", {
                data: { alertMsg: "서버오류: 잠시 뒤 다시 시도 해주세요" },
              });
            });
        } else {
          res.render("index.ejs", {
            data: { alertMsg: "글이 없거나 글쓴이가 일치하지 않습니다 " },
          });
        }
      })
      .catch((err) => {
    ...
    mongodb
      ...
      .then((result) => {
        ...
        if (result && result.id == req.session.user.userid) {
          ...
          res.render("index.ejs", {
            data: { alertMsg: "글이 없거나 글수정자가 일치하지 않습니다 " },
          });
        }
      })
      .catch((err) => {
...
router.post("/post/save", async (req, res) => {
  ...
  if (req.session.user) {
    if (req.session.user.userid == req.body.id) {
      console.log("req.body.frsc: ", req.body.frsc);
      console.log("req.session.csrf_token: ", req.session.csrf_token);
      if (
        typeof req.session.csrf_token != "undefined" &&
        typeof req.body.frsc != "undefined" &&
        req.session.csrf_token == req.body.frsc
      ) {
        //csrf_token이 맞으면
        const { mongodb } = await setup();
        ...
        res.render("index.ejs", {
          data: { alertMsg: "※Detect CSRF" },
        });
      }
    } else {
      console.log("글쓰기 해킹 시도 발생...");
      res.render("index.ejs", {
        data: { alertMsg: "로그인 사용자와 글작성자가 일치하지 않습니다 " },
      });
    }
  } else {
    res.render("index.ejs", { data: { alertMsg: "로그인 먼저 해주세요" } });
...
router.get("/post/enter", function (req, res) {
  if (req.session.user) {
    ...
    res.render("post/enter.ejs", {
      data: { id: req.session.user.userid },
...
function list(mongodb, req, res, searchQuery = {}) {
...
router.get("/search", async (req, res) => {
  if (req.session.user) {
    const { mongodb } = await setup();
    const searchQuery = { title: { $regex: req.query.value, $options: "i" } };
    list(mongodb, req, res, searchQuery);
  } else {
    res.render("index.ejs", { data: { alertMsg: "로그인 먼저 해주세요" } });
  }
});
